...
library("GO.db")
opt_list <- list(
  make_option("--all_genes", type = "character",
  help = "directory containing fisher tables"),
  make_option("--GO_annotations", type = "character",
...
# ----
GOdata <- new("topGOdata", ontology = "MF", allGenes = gene_enrichment3,
geneSel = function(p) p < 0.05, description = "Test", annot = annFUN.gene2GO,
gene2GO = GO_relationships)
GOdata
sg <- sigGenes(GOdata)
str(sg)
numSigGenes(GOdata)
...
showSigOfNodes(GOdata, score(resultFisher), firstSigNodes = 10,
useInfo = "all")
...
# ----
GOdata <- new("topGOdata", ontology = "BP", allGenes = gene_enrichment3,
geneSel = function(p) p < 0.05, description = "Test", annot = annFUN.gene2GO,
gene2GO = GO_relationships)
GOdata
sg <- sigGenes(GOdata)
...
# ----
GOdata <- new("topGOdata", ontology = "CC", allGenes = gene_enrichment3,
geneSel = function(p) p < 0.05, description = "Test", annot = annFUN.gene2GO,
gene2GO = GO_relationships)
GOdata
sg <- sigGenes(GOdata)
