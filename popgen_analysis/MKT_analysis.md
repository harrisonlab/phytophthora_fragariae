...
$input/SNP_calling/polished_contigs_unmasked_filtered_vcf_aa.vcf \
...
```
Out of 359,325 variants in the file, 240,975 were annotated with (an) ancestral allele(s)
```
...
```bash
python $scripts/compare_outgroup_results.py \
$input/summary_stats/polished_contigs_UK123_plus_ancestral_unmasked_filtered_vcf_aa.vcf \
$input/summary_stats/polished_contigs_UK123_plus_ancestral_unmasked_filtered_gen_aa.vcf 2 N
```

```
In total, 240,692 variants were annotated with consensus ancestral allele,
out of 358,599 variants in the file. 283 annotations were found to differ
between the two input annotation sources, and were rejected, while 117,624
and 0 ancestral allele annotations were present only in the input
