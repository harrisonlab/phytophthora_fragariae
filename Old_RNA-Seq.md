These commands were previously used for analysis of the new RNA-Seq data, before I realised I needed to change my approach, copied here for reference.
These should NOT be considered as commands run to produce the analysis results.
...
do
    ...
    do
        ...
        DEG_Files=$(ls alignment/star/P.fragariae/$Strain/DeSeq/*_vs_*.txt  | grep -v -e 'up' -e 'down' -e "CRN" -e "RxLR" -e "ApoP" | sed -e "s/$/ /g" | tr -d "\n")
...
do
    ...
    do
        ...
        do
            ...
            #	OrthoFile=$(ls analysis/orthology/orthomcl/Pcac_Pinf_Ppar_Pcap_Psoj/Pcac_Pinf_Ppar_Pcap_Psoj_orthogroups.txt)
            ProgDir=/home/adamst/git_repos/scripts/phytophthora_fragariae
            DEG_Files=$(ls alignment/star/P.fragariae/$Strain/DeSeq_"$Isolate"/*_vs_*.txt  | grep -v -e 'up' -e 'down' -e "CRN" -e "RxLR" -e "ApoP" | sed -e "s/$/ /g" | tr -d "\n")
            # $ProgDir/pacbio_anntoation_tables.py --gff_format gff3 --gene_gff $GeneGff --gene_fasta $GeneFasta --SigP2 $SigP2 --SigP4 $SigP4 --phobius $PhobiusTxt --RxLR_motif $RxLR_Motif --RxLR_Hmm $RxLR_Hmm --RxLR_WY $RxLR_WY --RxLR_total $RxLR_total --CRN_LFLAK $CRN_LFLAK --CRN_DWL $CRN_DWL --CRN_total $CRN_total --DEG_files $DEG_Files  > $OutDir/414_v2_gene_table_incl_exp.tsv
            # NormCount=$(ls alignment/star/P.cactorum/414_v2/DeSeq/normalised_counts.txt)
...
ProgDir=/home/adamst/git_repos/scripts/phytophthora_fragariae
Bc16_gene_table=gene_pred/annotation/P.fragariae/Bc16/Bc16_gene_table_incl_exp.tsv
Bc1_gene_table=gene_pred/annotation/P.fragariae/Bc16/Bc16_gene_table_incl_exp_Bc1.tsv
Nov9_gene_table=gene_pred/annotation/P.fragariae/Bc16/Bc16_gene_table_incl_exp_Nov9.tsv
OutDir=alignment/star/P.fragariae/Bc16/DeSeq/reference_guided_candidates
Bc16_output=$OutDir/Bc16_candidate_genes.txt
Bc1_output=$OutDir/Bc1_candidate_genes.txt
Nov9_output=$OutDir/Nov9_candidate_genes.txt
mkdir -p $OutDir
